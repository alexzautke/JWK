...
### Changed
- **Breaking:** the library targets .NET 10 (`net10.0`) instead of `netstandard2.0`. It can no longer be used from .NET Framework or from .NET (Core) versions before 10; stay on 0.7.1 for those.
...
- **Breaking:** `Serialize` and `Deserialize` are no longer public members of `Algorithm`, `KeyType` and `PublicKeyUse`. They implement the library's internal serialization contract and are now implemented explicitly, so they no longer show up as public API; `Deserialize(JObject)` only ever threw `NotImplementedException`. To migrate, replace `Serialize()` with `Algorithm.Name`, `KeyType.Type` or `PublicKeyUse.KeyUse`, and `Deserialize(token)` with `Algorithm.TryGetAlgorithm`, `KeyType.TryGetKeyType` or `PublicKeyUse.TryGetPublicKeyUse`. This breaks both binary and source compatibility for code which called these members.
...
- Reading and exporting JWKs is several times faster: the reflection data `JWKConverter` relies on is looked up once instead of on every read and export, and `JWKS.TryParse` no longer parses every key again. For a set of 20 RSA and EC keys, `JWKS.TryParse` went from about 13 ms to 1.5 ms and `JWKS.Export` from 6.4 ms to 1.2 ms.

### Fixed
- `new JWK(keyType, keyParameters, ...)` threw an `ArgumentNullException` when `keyOperations` was left at its default of null. A null `keyOperations` now means the JWK has no "key_ops", as it already did for the other constructors.
